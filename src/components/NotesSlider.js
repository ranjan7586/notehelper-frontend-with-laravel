...
const NotesSlider = () => {
    ...
    const getAllNotes = async () => {
        try {
            ...
            setNotes(data?.notes);
            console.log(data)
            getSimilarNotes(data?.notes[0]?._id, data?.notes[0]?.domain._id)
        } catch (error) {
            console.log(error);
        ...
        <>
            <div className="coffee_section layout_padding">
                <div className="container">
                    <div className="row">
                        <h1 className="coffee_taital">LATEST FROM {notes[0]?.domain?.name}</h1>
                        <div className="bulit_icon"><img src="Assets/images/bulit-icon.png" /></div>
                    </div>
                </div>
                <div className="coffee_section_2">
                    <div id="main_slider" className="carousel slide" data-ride="carousel" data-pause="false">
